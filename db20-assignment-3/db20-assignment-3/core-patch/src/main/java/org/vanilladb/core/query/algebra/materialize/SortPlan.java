...
public class SortPlan implements Plan {
	...
	public String explained(String explain_query_plan, int whiteSpace, boolean is_productplan) {
		String temp = "";
		for(int i=0; i<whiteSpace*4; i++) temp += " ";
		
		String sort_string = temp;
		sort_string += "->SortPlan: (#blks=";
		sort_string += this.blocksAccessed();
		sort_string += ", #recs=";
		sort_string += this.recordsOutput();
		sort_string += ")";
		sort_string += "\n";
		
		explain_query_plan += sort_string;
		
		return p.explained(explain_query_plan, whiteSpace+1, false);
	}
