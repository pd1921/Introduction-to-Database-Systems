...
public static final double READ_WRITE_TX_RATE;
	...
	static {
		NUM_ITEMS = BenchProperties.getLoader().getPropertyAsInteger( // getloader會在configuration 裡面的VM argument 讀取檔案
				As2BenchConstants.class.getName() + ".NUM_ITEMS", 100000);
		READ_WRITE_TX_RATE = BenchProperties.getLoader().getPropertyAsDouble(
				As2BenchConstants.class.getName() + ".READ_WRITE_TX_RATE", 5.0);
	}
