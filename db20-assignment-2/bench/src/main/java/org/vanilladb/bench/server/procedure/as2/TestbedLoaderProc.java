...
public class TestbedLoaderProc extends StoredProcedure<TestbedLoaderParamHelper> {
	...
	private void generateItems(int startIId, int endIId) {
		...
		for (int i = startIId; i <= endIId; i++) {
			...
			iimid = iid % (As2BenchConstants.MAX_IM - As2BenchConstants.MIN_IM) + As2BenchConstants.MIN_IM;   // generate the number between MAX_IM & MIN_IM
			iname = String.format("%0" + As2BenchConstants.MIN_I_NAME + "d", iid);  // if MIN_I_NAME is 2 ,this equal (%02d, iid) 
